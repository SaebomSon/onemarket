...
<style type="text/css">
   ...
   <script type="text/javascript">
	function pwCheck() {
		...
		if ( form1.pw.value != form1.checkpw.value) {
		...
		var num1 = 2;
		if(num1 >$("input:checkbox[name='interest']:checked").length){
			alert("관심분야는 2개이상 선택해주셔야 합니다!");
			return false;
		}
		
		
		var num2 = 0;
		if(num2 == $("input[name='check']:checked").length){
			alert("약관동의를 해주셔야 합니다!");
			return false;
		}
	...
	function openTextarea(check){
		if(check.checked==true) {
			//.animate({margin-left:250px,color:red},5000)
			//$("#myDiv").slideDown(500);
			myDiv.style.display='';
			$("#myDiv").animate({height:"1160px"},1000);
		}
		else {
			//$("#myDiv").slideUp(500);
			$("#myDiv").animate({height:"0px"},1000, function() {myDiv.style.display='none'; });
	...
	function openIdCheck() {
		window.name = "부모폼";
		window.open("buyerid_check_form.jsp", "checkForm", "width=300, height=200 top=330, left=930" );
	}
</script>
</head>
...
<body>
	<div id="header">
	<a href="ShopController?command=shopping_main"><p><span>오늘의 특가 상품은?!</span></p></a>
	</div>
	
	
	<div id="middlebg">
	<div class="login-form">
		<form class="form-horizontal" accept-charset="UTF-8" onsubmit="return pwCheck()" name="form1" action="ShopController?command=shopping_registerMember" method="post">
			<div class = "form-group"> 
				<a href="MainPage.jsp"><img src="example.jpg" width="80px" height="70px"></a>
				<!--  파일 크기 변경 해야함  -->
			</div>
			
			<div class="form-group">
				<label for="id">아이디: </label> 
				<input class="form-control" type="text" id="id" name="id" placeholder="ID" maxlength="14"/>
				<input type="button" value="중복체크" onclick="openIdCheck()"/>
			</div>

			<div class="form-group">
				<label for="pw">비밀번호: </label> <input  class="form-control" type="password" id="pw"
				placeholder="14자리까지 가능합니다"	name="pw"  maxlength="14"> 
			</div>

			<div class="form-group">
				<label for="pw-re">비밀번호 확인: </label> <input  class="form-control" type="password"
					id="checkpw" name="checkpw"  maxlength="14">
			</div>


			<div class="form-group">
				<label for="name">성함: </label> <input  class="form-control" type="text" id="name"
			...
			<div class="form-group">
				<label for="address">주소:</label><input type="text" id="address" name="address"
				 placeholder="주소입력란" style="width:320px; height:30px; font-size:12px;">
			...
			<div class="form-group">
				<label for="email">이메일: </label> <input  class="form-control" type="text" id="email"
					name="email" maxlength="14" /> <br>
			</div>

			<div class="form-group">
				<label for="phone">전화번호: </label> <input  class="form-control" type="text" id="phone"
				placeholder="-를 빼고 입력해주세요" name="phone" /> <br>
			</div>
			        
			<div class="form-group">
				<label for="interest">관심분야: </label> <input type="checkbox"
					name="interest" value="취업" />취업 <input type="checkbox"
					name="interest" value="운동" />운동 <input type="checkbox"
					name="interest" value="요리" />요리 <input type="checkbox"
					name="interest" value="음악" />음악 <br><input type="checkbox"
					name="interest" value="서적" />서적<input type="checkbox"
					name="interest" value="교양" />교양<input type="checkbox"
					name="interest" value="수험" />수험<input type="checkbox"
					name="interest" value="잡화" />잡화<input type="checkbox"
					name="interest" value="패션" />패션<input type="checkbox"
					name="interest" value="컴퓨터" />컴퓨터
			</div>
						
			<div class="form-group">
				<input type="checkbox"  name="check"/>약관동의
				<label for="agree">(필수)</label>
             
             <input type="checkbox" onClick="openTextarea(this)">펼치기
			 <div id="myDiv" style="display:none; height:0px;">
<pre> 			 
             제 1 장 총칙
	제 1 조 (목적)
...
1. 기업마당 사이트와 이용자 간에 발생한 서비스 이용에 관한 분쟁에 대하여는 대한민국 법을 적용하며, 본 분쟁으로 인한 소는 대한민국의 법원에 제기합니다.
			...
			<div class="form-group">
				<input type="submit"  class="btn btn-primary btn-block" value="작성완료">
			</div>
		</form>
	</div>
	</div>
